...
class CmsPage(db.Model):
    """Model for managing website pages"""
    ...
    layout = db.Column(db.String(50), default='full-width')
    ...
    # Author
    author_id = db.Column(db.Integer, db.ForeignKey('saas_users.id'), nullable=False)
    author = db.relationship('SaasUser', backref='authored_pages')

class CmsMedia(db.Model):
    """Model for managing media files"""
    ...
    # Upload info
    uploaded_by = db.Column(db.Integer, db.ForeignKey('saas_users.id'), nullable=False)
    uploader = db.relationship('SaasUser', backref='uploaded_media')

class CmsContent(db.Model):
    """Model for managing content blocks and components"""
    __tablename__ = 'cms_content'
    
    id = db.Column(db.Integer, primary_key=True)
    identifier = db.Column(db.String(100), unique=True, nullable=False)  # hero_section, about_text, etc.
    title = db.Column(db.String(200), nullable=False)
    content = db.Column(db.Text)
    content_type = db.Column(db.String(50), default='text')  # text, html, markdown, json
    
    # Content organization
    category = db.Column(db.String(100))  # homepage, about, services, etc.
    section = db.Column(db.String(100))   # header, main, sidebar, footer
    ...
    # Rich content data
    content_data = db.Column(db.JSON)  # For structured content
    ...
    # Author
    updated_by = db.Column(db.Integer, db.ForeignKey('saas_users.id'))
    updater = db.relationship('SaasUser', backref='updated_content')

class SiteSettings(db.Model):
    """Model for managing site-wide settings"""
    ...
    # Metadata
    category = db.Column(db.String(50))  # general, theme, contact, seo, social
    ...
    # Author
    updated_by = db.Column(db.Integer, db.ForeignKey('saas_users.id'))
    updater = db.relationship('SaasUser', backref='updated_settings')

class CmsMenu(db.Model):
    """Model for managing navigation menus"""
    __tablename__ = 'cms_menus'
    
    id = db.Column(db.Integer, primary_key=True)
    name = db.Column(db.String(100), nullable=False)
    location = db.Column(db.String(50))  # header, footer, sidebar
    
    # Timestamps
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    updated_at = db.Column(db.DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)

class CmsMenuItem(db.Model):
    """Model for menu items"""
    __tablename__ = 'cms_menu_items'
    
    id = db.Column(db.Integer, primary_key=True)
    menu_id = db.Column(db.Integer, db.ForeignKey('cms_menus.id'), nullable=False)
    parent_id = db.Column(db.Integer, db.ForeignKey('cms_menu_items.id'))
    
    title = db.Column(db.String(200), nullable=False)
    url = db.Column(db.String(500))
    page_id = db.Column(db.Integer, db.ForeignKey('cms_pages.id'))
    ...
    # Relationships
    menu = db.relationship('CmsMenu', backref='items')
    page = db.relationship('CmsPage', backref='menu_items')
    children = db.relationship('CmsMenuItem', backref=db.backref('parent', remote_side=[id]))
